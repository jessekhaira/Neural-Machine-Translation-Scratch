...
class GradientDescentMomentum(object):
    ...
    def __call__(self, learn_rate, params, dparams, grad_clip=False):
        ...
        output_params = []
        for i, param, dparam, running_avg in zip(range(len(params)), params,
                                                 dparams, self.running_avgs):
...
def crossEntropy(y, yhat, mask=None):
    ...
    return np.sum(loss, axis=0) / batch_size if mask is None else np.sum(
        mask.astype(int) * loss, axis=0) / batch_size
